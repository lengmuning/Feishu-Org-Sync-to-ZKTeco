...
CoreHR 事件只带 `employment_id` / `person_id` 时，Worker 会先把 `employment_id` 转成通讯录 `open_id` 并用 Directory 员工接口反查；如果反查不到，再用 CoreHR v2 员工接口按 ID 查询，并映射同样的 `pin/name/deptnumber`。
...
- `corehr:person:readonly` —— 按 `person_id` 查询个人信息关联的员工
...
实时事件验证：
- 在飞书改一个测试部门名称
- Cloudflare Dashboard → Worker → 实时日志 应在数秒内出现 `webhook ok contact.department.updated_v3 ...`
...
- **Webhook 异步 ACK**：Worker 收到事件后立即返回 200，实际写入在 `ctx.waitUntil` 后台完成，避免飞书超时重试
- **幂等性**：飞书可能重试同一事件，ZKTeco 的 update 接口按 `pin`/`deptnumber` upsert，天然幂等
- Worker subrequest 限制：免费版 50 次/调用，付费版 1000 次。Webhook 单次只调用 1-2 个 ZKTeco 接口，远低于上限；兜底全量约 20–25 次 subrequest，免费版可用
- Worker CPU 时间限制：免费版 10ms，付费版 30s。Webhook 单事件耗时极短，但**兜底全量同步约 15–25 秒**，**推荐 Workers Paid（$5/月）**
