...
基于 Cloudflare Worker 的单向同步服务，定时把飞书（Lark）组织架构的部门信息推送到 ZKTeco E-ZKEco PRO 门禁考勤系统。

## 功能

- **定时同步**：Cron 每 5 分钟自动执行一次
- **全量 upsert**：按 `deptnumber` 创建或更新部门，无需追踪增量
...
- **手动触发**：`GET /sync` 立即同步一次，`GET /preview` 仅预览映射结果不写入
...
- `contact:department.base:readonly` 或更高 —— 读取部门信息
- 应用需发布并获得企业管理员审批
...
curl https://<your-worker>.workers.dev/sync      # 立即同步
...
|---|---|---|
| `/` | GET | 帮助信息 |
| `/preview` | GET | Dry run，返回飞书映射结果和待标记的孤儿，不写 ZKTeco |
| `/sync` | GET | 立即执行完整同步 |
| （无） | scheduled | Cron `*/5 * * * *` 自动触发同步 |
...
- **员工信息不在本项目范围内**。同步只动部门表，不会创建、修改、删除任何员工记录
- 软停用的部门里原有员工保持在原位，避免误删人员数据
- 飞书限流：每 5 分钟一次同步对典型中小企业（<500 部门）不会触发限流；若部门数非常多或限流，可改为更低频率
- Worker CPU 时间限制：免费版 10ms，付费版可到 30s 或更多。本项目单次同步约 20 秒，**建议使用付费版或调整到更高的 CPU 限制**
